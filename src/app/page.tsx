...
async function loadEvaluations(): Promise<EvaluationRecord[]> {
  try {
    const rows = await db
      .select()
...
export default async function DashboardPage() {
  ...
  const average = total > 0 ? round1(scores.reduce((s, v) => s + v, 0) / total) : 0;
  ...
  return (
    <div className="space-y-10">
      ...
      <section className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
        ...
        <StatCard label="Average similarity" value={total ? formatScore(average) : "—"} />
        <StatCard label="Median similarity" value={total ? formatScore(med) : "—"} />
        <StatCard label="Highest similarity" value={total ? formatScore(highest) : "—"} />
        <StatCard label="Lowest similarity" value={total ? formatScore(lowest) : "—"} />
      </section>
        ...
        <section className="flex flex-col gap-4 lg:col-span-2">
          ...
          <div className="flex items-start gap-3 rounded-2xl border border-[var(--line)] bg-white p-5">
            ...
            <p className="text-xs leading-relaxed text-[var(--ink-soft)]">
              <strong className="text-[var(--ink)]">Privacy first.</strong> All
              image processing happens locally in the browser. Images are only
              sent to an AI service when you explicitly enable per-evaluation AI
              analysis.
            </p>
          </div>
        </section>
