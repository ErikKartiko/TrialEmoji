...
function ReportImage({ src, caption }: { src: string; caption: string }) {
  return (
...
/** Shared printable report markup (used for server-stored and local records). */
export function ReportDocument({ record: e }: { record: EvaluationRecord }) {
  const meta = CATEGORY_META[e.category];
  const hasAi = e.aiStatus === "ok" && e.aiScore !== null && e.aiShape !== null;
          ...
          <tbody>
            {[
              ...
              <tr key={k as string} style={{ borderTop: "1px solid #efece2" }}>
                ...
                <td style={{ padding: "7px 0", color: "#1c1a15", fontWeight: 500 }}>
                  {v}
                </td>
              </tr>
            ))}
